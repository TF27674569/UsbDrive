...
import org.usb.utils.CRC16X25Util;
...
import static org.usb.config.InstructConfig.*;
...
public class WorkDispatcher implements Handler.Callback {
    ...
    private void handleInstruct(byte[] instruct) {
        ...
        PrintUtils.printHex(instruct, "接收到的数据:");
        ...
        // 拿到数据的长
        int dataLength = PrintUtils.byteArrayToInt(int_len);
        ...
        PrintUtils.printHex(packageData, "校验数据:" + passCRC);
    ...
    private void circulationData(byte[] packageData) {
        ...
        for (Instruct instruct : sInstruct) {
            if (PrintUtils.equals(instruct.getIntercept(), packageData)) {
                tag = instruct;
                break;
            }
        }
